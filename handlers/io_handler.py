...
async def io_report(symbol: str | None = None):
    ...
    # Eğer coin seçilmemişse => Market raporu
    if symbol is None:
    ...
    for sym in symbols:
        try:
            ...
            snap = build_io_snapshot(sym, kl, ob, trades, ticker, funding[0] if funding else {})
            snapshots[sym] = snap
...
def format_io_market(snapshots: dict) -> str:
    ...
    """
    lines = []
    lines.append("⚡ Nakit raporu\n")
    avg_trend = safe_mean([snap["trend_score"] for snap in snapshots.values() if snap])
    avg_liq = safe_mean([snap["liquidity_score"] for snap in snapshots.values() if snap])
    avg_risk = safe_mean([snap["risk_score"] for snap in snapshots.values() if snap])
...
def register_io_handlers(dp):
    dp.add_handler(CommandHandler("io", lambda u, c: asyncio.run(io_cmd(u, c))))
